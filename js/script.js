...
var AreaModel = function() {
  ...
  */
  this.isBlankDay = function(currentDate) {
    ...
    if (
      period[0].getTime() <= currentDate.getTime() &&
...
var TrashModel = function(_lable, _cell, remarks) {
  ...
  for (var j in this.dayCell) {
    if (this.dayCell[j].length == 1) {
      ...
      this.dayCell[j].length == 2 &&
      this.dayCell[j].substr(0, 1) != "*"
    ) {
      result_text +=
        "第" + this.dayCell[j].charAt(1) + this.dayCell[j].charAt(0) + "曜日 ";
    } else if (
      this.dayCell[j].length == 2 &&
      this.dayCell[j].substr(0, 1) == "*"
    ) {
    } else if (
      this.dayCell[j].length == 10 &&
      this.dayCell[j].substr(0, 1) == "隔"
    ) {
  ...
  this.getDateLabel = function() {
    ...
    var result_text =
      this.mostRecent.getFullYear() +
      "/" +
      (1 + this.mostRecent.getMonth()) +
      "/" +
      this.mostRecent.getDate();
            ...
            d.setTime(
              date.getTime() +
                1000 *
                  ...
                  60 *
                  24 *
                  ((7 + getDayIndex(day_mix[j].charAt(0)) - date.getDay()) %
                    7) +
    ...
    csvToArray("data/area_days.csv", function(tmp) {
      ...
      csvToArray("data/center.csv", function(tmp) {
        ...
        for (var row_index in areaModels) {
          ...
          var selected =
            selected_name == area_name ? 'selected="selected"' : "";
    ...
    for (var i in areaModel.trash) {
      ...
      for (var d_no in descriptions) {
        ...
        for (var j in targets) {
          ...
          target_tag +=
            '<li style="list-style:none;"><div>' + target.name + "</div>";
        ...
        if (trash.mostRecent === undefined) {
          ...
          var leftDay = Math.ceil(
            (trash.mostRecent.getTime() - today.getTime()) /
              (1000 * 60 * 60 * 24)
          );
        ...
        accordionHTML +=
          ...
          '<a class="accordion-toggle" style="height:' +
          accordion_height +
          'px" data-toggle="collapse" data-parent="#accordion" href="#collapse' +
          i +
          '">' +
          '<div class="left-day">' +
          leftDayText +
        ...
        if (ableSVG && SVGLabel) {
          ...
          accordionHTML +=
            '<p class="text-center">' + description.label + "</p>";
        }
        accordionHTML +=
          "</div>" +
